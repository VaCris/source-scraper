...
const detectMediaType = (jsonLd, $) => {
  ...
  if (types.some((type) => ["tvseries", "tvseason", "tvepisode", "series"].includes(type))) return "series";
...
\r\t\s"'<>]+/gi;
  for (const match of rawHtml.matchAll(urlPattern)) {
...
const collectRelatedPages = ($, pageUrl) => {
  ...
  $("a[href]").each((_, element) => {
    ...
    if (!episode) return;
    links.set(url, { pageUrl: url, season, episode, label: text || null });
  });
...
export const parseGenericPage = ({ html, pageUrl }) => {
  ...
  return {
    ...
    jsonLdTypes: [...new Set(jsonLd.flatMap((item) => Array.isArray(item?.["@type"]) ? item["@type"] : [item?.["@type"]]).filter(Boolean))],
  };
};
